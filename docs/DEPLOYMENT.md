...
Ожидаемая версия Node.js: `v24.21.0`. `git status --short` не должен показывать незакоммиченные исходники.
...
На VPS должна быть та же patch-версия Node.js: `v24.21.0`. Если Node отсутствует или отличается, сначала установите именно эту версию утверждённым системным способом. Не продолжайте с неподдерживаемой версией.
...
После первого запуска приложение сохранит исходный `system_prompt.md` в `prompt-history/` как версию и создаст там `active.json`. Следующие версии загружайте через настройки администратора; там же выбирайте текущую версию. После появления `active.json` ручная замена `system_prompt.md` не переключает активный промпт. Для переноса выбранных версий между серверами переносите весь каталог `prompt-history/` защищённым способом вместе с файлом `active.json`; сохраняйте права доступа сервисного пользователя. Ни один файл этого каталога, включая `active.json`, не должен попадать в GitHub. Эти файлы входят только в общую резервную копию VPS, отдельного резервирования приложение не делает.
...
Real-LLM проверка расходует средства KodikRouter и выполняется только явно.
