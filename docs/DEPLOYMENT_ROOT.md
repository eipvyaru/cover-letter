...
Чтобы ограничить автоматическую загрузку источников только HeadHunter, задайте в локальном `.env` перед синхронизацией:

```env
SOURCE_ALLOWED_HOSTS=hh.ru,*.hh.ru
```

Пустое или отсутствующее значение сохраняет поддержку любых публичных доменов. В обоих режимах приватные адреса блокируются при DNS-проверке и непосредственно при сетевом соединении.
