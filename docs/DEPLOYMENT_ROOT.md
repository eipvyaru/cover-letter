...
Чтобы разрешить автоматическую загрузку сайтов в зоне `.ru`, у которых доменное имя начинается с латинской буквы и далее содержит латинские буквы, цифры или внутренние дефисы, задайте в локальном `.env` перед синхронизацией:

```env
SOURCE_ALLOWED_HOST_MASKS_JSON='["^(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?[.])*[A-Za-z](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?[.]ru$"]'
```

Значение является JSON-массивом регистронезависимых anchored-regex масок. Каждая маска обязана начинаться с `^` и заканчиваться `$`. Указанная `.ru` маска должна оставаться первым элементом; дополнительные маски добавляются отдельными строками, например `SOURCE_ALLOWED_HOST_MASKS_JSON='["^(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?[.])*[A-Za-z](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?[.]ru$","^(?:[A-Za-z0-9-]+[.])*example[.]com$","^hh[.]ru$"]'`. Первой маске соответствуют `hh.ru`, `api.hh.ru`, `a1.ru`, `a-b.ru` и `www.a-b.ru`; не соответствуют `1a.ru`, `-ab.ru`, `ab-.ru`, `example.com` и `example.ru.evil.com`. Пустое или отсутствующее значение сохраняет поддержку любых публичных доменов.

Совпадение с маской, в том числе с широкой маской для `*.ru` или `*.com`, не отменяет SSRF-проверку IP. После allowlist приложение проверяет все A/AAAA ответы DNS и повторяет проверку непосредственно при сетевом соединении. Даже разрешённый hostname блокируется, если DNS указывает на `127.0.0.1`, `10.0.0.0/8`, `::1`, link-local, metadata endpoint или другую непубличную сеть.
