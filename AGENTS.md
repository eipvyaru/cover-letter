...
- The script must verify Node.js `v24.21.0`, check the main worktree and sensitive-file tracking, then run `npm ci`, `npm audit --audit-level=moderate`, migrations, lint, typecheck, tests, and the production build in a temporary clean Git worktree so an active development server cannot lock dependencies.
