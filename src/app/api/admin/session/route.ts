...
export async function POST(request:Request){
 ...
 try{const text=await request.text();if(text.length>2000)throw new Error('large');const body=JSON.parse(text) as {login?:unknown;password?:unknown};if(typeof body.login!=='string'||typeof body.password!=='string'||body.login.length>200||body.password.length>500)throw new Error('invalid');const settings=runtimeSettings();if(!await verifyAdminCredentials(body.login,body.password,settings)){audit('admin_login_failed',body.login,request);return Response.json({error:'Неверный логин или пароль.'},{status:401});}audit('admin_login',body.login,request);return Response.json({isAdmin:true},{headers:{'Set-Cookie':await createAdminCookie(settings,request),'Cache-Control':'no-store'}});}catch{return Response.json({error:'Проверьте логин и пароль.'},{status:400});}
}
